...
## ADR-034 — Follow-up chips, and the KB is why they are not generated

**Status:** accepted · 2026-09-18

**Context.** Every answer ended and nothing happened next. The reviewer's words:
"estímulo, reacción, punto… no me invita a seguir usandolo." Correct, and I had
invented a worse version of a solved problem instead of looking it up, which CLAUDE.md
explicitly forbids ("Research before inventing… don't reinvent the wheel badly").

**What the industry does.** Qwairy's GEO study (2026-06-15,
https://www.qwairy.co/blog/ai-follow-up-suggestions-june-2026) measured closing
behaviour across assistants: Perplexity ends ~77% of answers with an offer to continue,
ChatGPT ~67%, roughly double eight months earlier. Google's AI Overviews sit at ~0% —
a snippet, not a conversation. This bot was at the AI Overviews end.

The shape matters as much as the presence. `laguagu/claude-code-nextjs-skills`,
`skills/nextjs-chatbot/suggestions.md` (read 2026-09-18) names the trap directly: a
blanket "suggest three follow-up questions" rule fires even when the bot has just asked
the visitor something, so the visitor is handed three questions to ask instead of the
one answer they were meant to give. Its other applicable findings: classify the answer's
tail with a small function rather than another model call; lay the chips out in a
wrapping flex container, never a horizontal scroller, because whole-question chips vary
in length and a scroller hides the long ones behind a fade.

**Decision.** Two or three chips after an answer, derived from the compiled knowledge
base rather than generated by a model, and suppressed entirely when the bot's own answer
ended in a question.

**Why derived and not generated,** in order of weight:
1. A generated chip can propose a question about something Cadre does not publish, and
   the bot would then have to refuse its own suggestion. That is worse than no chip, and
   not-inventing is this product's whole reason to exist.
2. A second model call per turn is real money against a $5/7-day budget, for a feature
   that is by definition non-critical.
3. A pure function is testable with no network, which nothing about the generated
   version would be.

**Consequences.** `scripts/walkthrough.ts` now fails if any turn is a dead end — no chip
to click, no question from the bot, no offer on the table. That check, not a passing
eval, is what would have caught this.

**What it cost to learn:** the reviewer tested the deployment repeatedly and found it
unusable each time, while a 37-case suite reported 35 passes. Those cases ask one
question each and never look at the second turn.
